////////////////////////dev flags////////////////////////
const bool viewPassword = false; //will view WiFi password in logs
const bool skipNetworkScan = false;
const bool skipClockSync = false;
const bool skipDBRequest = false;
//#define NOLOG
//#define LOGHTTPONLY
#define COMBINELOG
#define BUFLOGSIZE 5000 //max size of log.txt (log.txt is stored in RAM)
#define CoreMenu // show core menu on website
#define OTA //enables OTA update (insecure)
///////////////////////dev flags////////////////////////


////////////////////////setup flags////////////////////////
const byte sensor = D5;    //flow sensor input pin on the Arduino
const byte Clock = D7;
const byte statusLed = D0; //status led pin
const byte warningLed = D8; //storage status led
const byte button = D6; // to reset WIFI settings connect it to ground before system starts; to sync RTC connect to GROUND while system running
const String serverName  = "https://wrzaskowski.ct8.pl/api/";  // db scripts adress
const char APname [] = "water flow sensor";
const char APpassword [] = "xd12134567890";
const char ntpServerName [] = "tempus2.gum.gov.pl"; // official polish time source
const String recordsFolder = "/records" ; //in chip memory
const unsigned int recordsLimit = 32;
const IPAddress apIP(172, 217, 28, 1);
const IPAddress subnetmask(255, 255, 255, 0);
const bool apPassword = true;
const float sensorStep = 2.25;
const byte RTCaddress = 0x68; //on I2C bus
const char ISRG_Root_X1 [] PROGMEM = R"CERT(
-----BEGIN CERTIFICATE-----

-----END CERTIFICATE-----
)CERT";//Internet Security Research Group certificate valid to Mon, 04 Jun 2035 11:04:38 GMT
//authorising Let's Encrypt
////////////////////////setup flags////////////////////////
